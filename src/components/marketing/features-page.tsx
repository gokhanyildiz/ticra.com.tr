...
export function FeaturesPage({ locale, dict }: FeaturesPageProps) {
  ...
  const pricingCopy =
    locale === 'tr'
      ? {
          overline: 'Ekipler için kullanım',
          title: 'Ayrı işler aynı güncel bilgilerle ilerler',
          description:
            'Satış, satın alma, stok, finans, servis, proje, İK, doküman ve sözleşme süreçleri birbirini tamamlayacak şekilde tasarlanır.',
        }
      : {
          overline: 'Team use',
          title: 'Separate jobs move with the same up-to-date information',
          description:
            'Sales, purchasing, inventory, finance, service, projects, HR, documents and contracts are designed to support each other.',
        };
  const includedTitle =
    locale === 'tr' ? 'Ticra ile yönetebileceğiniz işler' : 'What you can manage with Ticra';
  const includedSubtitle =
    locale === 'tr'
      ? 'Satış, satın alma, stok, finans, proje, servis, İK, doküman, sözleşme, demirbaş, rapor ve çoklu firma yönetimi aynı ürün deneyiminde toplanır.'
      : 'Sales, purchasing, inventory, finance, projects, service, HR, documents, contracts, assets, reports and multi-company management are brought together in one product experience.';
